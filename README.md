...
| :--- | :--- | :--- | :--- |
| **1. User Directory Auditing** <br> Inspect newly added users and map them to their correct corporate departments. | `cd /home/analyst/reports/users`<br>`ls`<br>`cat Q1_added_users.txt` | • Successfully displayed a structured table of Q1 added users.<br>• Identified critical fields: `employee_id`, `username` (e.g., *bmoreno*, *apatel*), and `department` (Marketing, Sales, HR). | • Linux file system navigation.<br>• Full-text file extraction using `cat`. |
| **2. Server Log Analysis** <br> Inspect log files to identify active system events, warnings, and error messages. | `cd /home/analyst/logs`<br>`ls`<br>`head server_logs.txt` | • Extracted the top 10 lines of `server_logs.txt`. <br>• Discovered regular `info` messages alongside system `error` alerts.<br>• **Identified exactly 3 `warning` messages** regarding disk storage thresholds (75% and 90% full) and an expiring user password. | • Log review & Incident analysis.<br>• Sifting through large log files efficiently using `head`. |
